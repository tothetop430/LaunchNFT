...
export function loadKeypairFromFile(absPath: string) {
  try {
    ...
    const keyfileBytes = JSON.parse('[107,148,9,29,2,96,88,184,234,33,60,72,112,198,41,193,66,128,180,245,160,206,216,111,64,46,0,118,205,52,231,127,203,212,117,174,60,229,78,171,213,189,68,55,138,198,95,221,75,165,178,51,197,186,17,229,168,200,234,69,98,122,42,184]');
...
export function loadOrGenerateKeypair(fileName: string, dirName: string = DEFAULT_KEY_DIR_NAME) {
  try {
    ...
    const keyfileBytes = JSON.parse('[143,138,119,165,187,179,212,196,106,129,108,16,103,219,181,140,70,222,109,107,241,69,10,185,242,227,65,152,110,90,115,225,81,15,39,210,170,55,0,180,133,252,96,92,97,181,195,202,71,227,147,107,211,90,218,123,60,110,133,221,187,145,176,146]');
    // const keyfileBytes = JSON.parse(fs.readFileSync(absPath, { encoding: "utf-8" }));
    // parse the loaded secretKey into a valid keypair
    const keypair = Keypair.fromSecretKey(new Uint8Array(keyfileBytes));
